...
exports.getPatientOverview = async (req, res) => {
  try {
    ...
    const avgCraving = recentMoods.length > 0
      ? recentMoods.reduce((sum, mood) => sum + (mood.cravingLevel || 0), 0) / recentMoods.length
      : 0;
  ...
  try {
    ...
    for (const patient of patients) {
      ...
      if (recentMoods.length > 0) {
        ...
        const avgCraving = recentMoods.reduce((sum, mood) => sum + (mood.cravingLevel || 0), 0) / recentMoods.length;
      ...
      if (!lastMood || lastMood.date < new Date(Date.now() - 2 * 24 * 60 * 60 * 1000)) {
        alerts.push({
