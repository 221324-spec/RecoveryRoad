...
export default function GoalsDashboard() {
  ...
  const fetchGoals = useCallback(async () => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/goals/${endpoint}`, {
  ...
  const toggleMilestone = async (goalId, index) => {
    try {
      const res = await fetch(`${API_BASE}/api/goals/${goalId}/milestone`, {
        method: 'POST',
        headers: {
  ...
  const downloadCertificate = async (goalId, title) => {
    try {
      const res = await fetch(`${API_BASE}/api/goals/${goalId}/certificate`, {
